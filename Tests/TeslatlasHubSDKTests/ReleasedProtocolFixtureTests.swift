...
final class ReleasedProtocolFixtureTests: XCTestCase {
  func testFixturesArePinnedToExactReleasedAuthority() throws {
    ...
    XCTAssertEqual(
      authority,
      "teslatlas-protocol 53b5c6483990db84e5214176755f398e93d87b1b profile 1.2.0"
    )
  }
  ...
  func testFixtureBytesMatchReleasedAuthorityHashes() throws {
    ...
    for name in ReleasedFixture.protocolFixtureNames {
      let digest = PortableSHA256.hexDigest(
        of: try ReleasedFixture.data(name)
      )
...
private enum PortableSHA256 {
  ...
  private static func digest(of data: Data) -> [UInt8] {
    ...
    for offset in stride(from: 0, to: message.count, by: 64) {
      ...
      for index in 0..<64 {
        ...
        let choose = (e & f) ^ ((~e) & g)
        let t1 = h &+ sum1 &+ choose &+ constants[index] &+ words[index]
